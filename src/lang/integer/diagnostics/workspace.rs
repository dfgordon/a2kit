...
impl WorkspaceScanner {
    pub fn new() -> Self {
        ...
        parser.set_language(&tree_sitter_integerbasic::LANGUAGE.into()).expect(RCH);
        Self {
            parser,
            line: String::new(),
    ...
    /// Buffer all documents matching `*.bas` or `*.ibas` in any of `dirs`, up to maximum count `max_files`,
...
impl Navigate for WorkspaceScanner {
    ...
    fn visit(&mut self, curs: &TreeCursor) -> Result<Navigation,DYNERR> {
        ...
        let curr = curs.node();
        if let Some(nav) = super::pass1::visit_defs_and_decs(&mut self.working_symbols, curs, &self.line, self.curr_row, self.in_dim_statement) {
            return Ok(nav);
        }
        // find the CHAIN patterns
        if curr.kind() == "statement_print_str" {
            if let Some(prog) = super::chain::test_chain(&curr,&self.line) {
                let emu_path = [&prog,".bas"].concat();
                let set1 = lang::get_emulation_match(&self.ws.docs, &emu_path, "/");
                let emu_path = [&prog,".ibas"].concat();

    ...
    pub fn get_from_node(&self,curs: &tree_sitter::TreeCursor,line: &str) -> Option<String> {
        ...
        if curs.node().kind() == "int" {
            ...
            if let Some(cmd) = maybe_cmd {
                if cmd.kind() == "tok_poke" || cmd.kind() == "tok_peek" || cmd.kind() == "tok_call" {

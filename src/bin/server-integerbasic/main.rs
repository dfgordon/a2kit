...
//! This is the Integer BASIC language server.
...
fn parse_configuration(resp: lsp_server::Response) -> Result<integer::settings::Settings,Box<dyn Error>> {
    if let Some(result) = resp.result {
        if let Some(ary) = result.as_array() {
            ...
            for item in ary {
                ...
                match integer::settings::parse(&json_config) {
...
struct Tools {
    config: integer::settings::Settings,
    thread_handles: VecDeque<std::thread::JoinHandle<Option<AnalysisResult>>>,
    doc_chkpts: HashMap<String,integer::checkpoint::CheckpointManager>,
    analyzer: Arc<Mutex<integer::diagnostics::Analyzer>>,
    hover_provider: integer::hovers::HoverProvider,
    completion_provider: integer::completions::CompletionProvider,
    highlighter: integer::semantic_tokens::SemanticTokensProvider,
    tokenizer: integer::tokenizer::Tokenizer,
    disk: DiskServer
}
...
impl Tools {
    pub fn new() -> Self {
        Self {
            config: integer::settings::Settings::new(),
            thread_handles: VecDeque::new(),
            doc_chkpts: HashMap::new(),
            analyzer: Arc::new(Mutex::new(integer::diagnostics::Analyzer::new())),
            hover_provider: integer::hovers::HoverProvider::new(),
            completion_provider: integer::completions::CompletionProvider::new(),
            highlighter: integer::semantic_tokens::SemanticTokensProvider::new(),
            tokenizer: integer::tokenizer::Tokenizer::new(),
            disk: DiskServer::new()
        }
    }
}
...
fn main() -> Result<(), Box<dyn Error + Sync + Send>> {
    ...
    let result = lsp::InitializeResult {
        capabilities: lsp::ServerCapabilities {
            ...
            // The following is not used because we are using the client side provider
            ...
            //         token_types: integer::semantic_tokens::TOKEN_TYPES.iter().map(|x| lsp::SemanticTokenType::new(x)).collect(),
            //         token_modifiers: vec![]
            //     },
            //     range: None,
    ...
    let req = lsp_server::Request::new(
        lsp_server::RequestId::from("integerbasic-reg-config".to_string()),
